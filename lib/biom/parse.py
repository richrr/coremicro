...
def convert_biom_to_table(biom_f, header_key=None, header_value=None, \
    ...
    if md_format is None:
        ...
        # to avoid every character in taxonomy being split and joined by '; '
        # the below cmd ('lambda x: x') is actually being sent from convert_biom.py method call
        # so the enclosing if condition is avoided altogether 
        #md_format = lambda x: x 
